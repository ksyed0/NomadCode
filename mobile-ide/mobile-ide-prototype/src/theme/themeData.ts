/**
 * Pure theme data — no hooks, no store imports.
 *
 * Extracted from tokens.ts so that monacoThemes.ts (and any other pure
 * data consumer) can import THEMES, ThemeId, and ThemeTokens without
 * pulling in useSettingsStore → AsyncStorage transitively.
 *
 * tokens.ts re-exports everything from here so existing callers are unaffected.
 */
